...
import { ParticleField } from "@/components/neometo/particle-field";
...
export function Hero() {
  return (
    <section id="top" className="relative overflow-hidden pb-20 pt-16 md:pb-28 md:pt-24">
      ...
      <ParticleField className="absolute inset-x-0 top-10 mx-auto hidden h-[360px] max-w-5xl opacity-70 md:block" />
      <div className="section-shell relative">
        <div className="max-w-3xl animate-rise">
          <p className="eyebrow">NEOMETO — New Method</p>
          <h1 className="mt-6 text-[2.6rem] font-extrabold leading-[1.02] text-ink sm:text-6xl lg:text-7xl">
          ...
          <p className="mt-8 text-sm font-medium text-success">No diagnosis required.</p>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section id="methods" className="scroll-mt-24 py-20 md:py-28">
      <div className="section-shell">
        <h2 className="max-w-2xl text-3xl font-extrabold text-ink md:text-5xl">
        ...
        <ul className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {problems.map((problem) => (
            <li key={problem.title}>
              <button
                type="button"
                className="group relative flex h-full w-full flex-col items-start overflow-hidden rounded-3xl border border-border bg-surface p-7 text-left shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-brand/40 hover:shadow-lift focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              >
                <span className="absolute -right-10 -top-10 size-28 rounded-full bg-brand-soft opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                <span className="relative text-xl font-bold text-ink">{problem.title}</span>
                <span className="relative mt-2 text-sm text-muted-foreground">{problem.hint}</span>
...
export function Manifesto() {
  return (
    <section className="py-20 md:py-28">
      <div className="section-shell grid gap-12 lg:grid-cols-[1.1fr_1fr] lg:items-center">
        <h2 className="text-3xl font-extrabold leading-tight text-ink md:text-5xl">
      ...
      <div className="section-shell">
        ...
        <ol className="mt-12 grid gap-6 md:grid-cols-3">
          {steps.map((step) => (
            <li
              key={step.number}
              className="rounded-3xl border border-border bg-surface p-8 shadow-soft transition-transform duration-300 hover:-translate-y-1"
            >
              <span className="block text-5xl font-extrabold tracking-tight text-brand-soft">
                {step.number}
              </span>
              <p className="mt-6 text-xl font-bold text-ink md:text-2xl">{step.title}</p>
            </li>
          ))}
        </ol>
...
export function FeaturedMethod() {
  return (
    <section className="py-20 md:py-28">
      <div className="section-shell">
        <article className="group grid gap-10 overflow-hidden rounded-4xl border border-border bg-surface p-8 shadow-soft transition-shadow duration-500 hover:shadow-lift md:grid-cols-[1fr_0.8fr] md:items-center md:p-14">
          <div>
            <p className="eyebrow">Featured method</p>
            <h2 className="mt-5 text-3xl font-extrabold text-ink md:text-5xl">Thought Spin</h2>
            <p className="mt-3 text-sm font-semibold text-brand">2–3 minutes</p>
            <p className="mt-6 text-xl font-medium text-ink md:text-2xl">
  ...
  return (
    <section id="advisor" className="scroll-mt-24 py-20 md:py-28">
      <div className="section-shell grid gap-12 lg:grid-cols-2 lg:items-center">
        <div>
          <h2 className="text-3xl font-extrabold text-ink md:text-5xl">
        ...
        <div className="rounded-4xl border border-border bg-surface p-6 shadow-soft md:p-8">
          <div className="flex justify-end">
            <p className="max-w-[80%] rounded-3xl rounded-br-lg bg-ink px-5 py-4 text-sm text-background md:text-base">
              I need to sleep but my thoughts won&apos;t stop.
            </p>
          </div>
          <div className="mt-4 flex justify-start">
            <p className="max-w-[80%] rounded-3xl rounded-bl-lg bg-brand-soft px-5 py-4 text-sm text-ink md:text-base">
...
export function Philosophy() {
  return (
    <section id="philosophy" className="scroll-mt-24 py-20 md:py-28">
      <div className="section-shell rounded-4xl bg-ink px-7 py-16 text-background md:px-16 md:py-24">
        <h2 className="max-w-3xl text-3xl font-extrabold leading-tight md:text-5xl">
          You don&apos;t need a label to use a useful method.
        </h2>
        <div className="mt-8 max-w-2xl space-y-4 text-base opacity-75 md:text-lg">
          <p>
            NEOMETO starts with what you want to handle better — not what somebody calls it.
          </p>
          <p>
...
export function Coming() {
  return (
    <section id="coming" className="scroll-mt-24 py-20 md:py-28">
      <div className="section-shell">
        <h2 className="text-3xl font-extrabold text-ink md:text-5xl">One method becomes many.</h2>
        <ul className="mt-12 grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          {areas.map((area) => (
            <li
              key={area}
              className="rounded-2xl border border-border bg-surface px-5 py-8 text-center text-sm font-semibold text-ink shadow-soft transition-transform duration-300 hover:-translate-y-1"
            >
              {area}
            </li>
          ))}
  ...
  return (
    <section id="final-cta" className="scroll-mt-24 py-24 md:py-36">
      <div className="section-shell relative text-center">
        ...
        <h2 className="relative text-3xl font-extrabold text-ink md:text-6xl">
          What could you handle better today?
        </h2>
        <Button asChild size="lg" className="relative mt-10 rounded-full px-10 text-base">
